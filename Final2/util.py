...
def draw_bboxes3(img, bbox, identities, classname):
    for i,box in enumerate(bbox):
        x1,y1,x2,y2 = [int(i) for i in box]
        track_id = int(identities[i]) if identities is not None else 0  
        
        class_name = int(classname[i]) if classname is not None else 0    
        ...
        #cv2.rectangle(img, (int(x1), int(y1-30)), (int(x1)+(len(label)+len(str(track_id)))*17, int(y1)), color, -1)
        cv2.putText(img, label + "-" + str(track_id),(int(x1), int(y1-5)),0, 0.5, (255,255,255),1)
        #cv2.putText(img,label + "-" + str(track_id),(int(x1), int(y1-10))),cv2.FONT_HERSHEY_SIMPLEX,0.45,(255,255,255)
    return img



def draw_bboxes2(img, bbox, identities, classname):
    for i,box in enumerate(bbox):
        x1,y1,x2,y2 = [int(i) for i in box]
        track_id = int(identities[i]) if identities is not None else 0    
        ...
        cv2.rectangle(img, (int(x1), int(y1)), (int(x2), int(y2)), color, 2)
        cv2.rectangle(img, (int(x1), int(y1-30)), (int(x1)+(len(label)+len(str(track_id)))*17, int(y1)), color, -1)
        cv2.putText(img, label + "-" + str(track_id),(int(x1), int(y1-10)),0, 0.75, (255,255,255),2)
