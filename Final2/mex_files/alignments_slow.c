...
/*                                    Matlab interface                        */
/*----------------------------------------------------------------------------*/
extern void  mexFunction_alignment_slow(double * input_points, int X_in, int N_in, double ** output_points, int *X_out, int *N_out);

void  mexFunction_alignment_slow(double * input_points, int X_in, int N_in, double ** output_points, int *X_out, int *N_out){
  /* 
     input: list of 2D points
     output: 7xN matrix with info for N alignment detections
  */
    /* input_points input points of size 2*N first N
  ...
  // if(sizeof(all_alignments->array)/sizeof(all_alignments->array[0]) == 0){
  //   all_alignments->pos = 0;
  // }
  // printf("before sorting all_alignments pos is %d, array size is %d\n", all_alignments->pos,sizeof(all_alignments->array)/sizeof(all_alignments->array[0]));
  
  /* sort alignements */
  qsort ((*all_alignments).array, all_alignments->pos,
         sizeof(struct point_alignment), compare_alignments);

  // printf("after sorting all_alignments pos is %d, array size is %d\n", all_alignments->pos,sizeof(all_alignments->array)/sizeof(all_alignments->array[0]));
  ...
  int n_out = (int)(*f1v1_alignments).pos;
  // if(n_out == 0)
  //   n_out = 1;
  ...
  // printf("out array assignment loop finished \n");
  
  // printf("out array print loop %d\n", n_out);
  // for (int i=0; i < 6 * n_out; ++i)
  //   printf("%f ", outArray[i]);
  // printf("\n");
  ...
  if(best_align.l != NULL) free_ntuple_list(best_align.l);
  // printf("freeing free_ntuple_list finished \n");

  // printf("all_alignments pos is %d, array size is %d\n", all_alignments->pos,sizeof(all_alignments->array)/sizeof(all_alignments->array[0]));
  free_alignment_list(all_alignments);
  // printf("freeing all_alignments finished\n");
  
  
  
  free_alignment_list(f1v1_alignments);
  // printf("freeing f1v1_alignments finished\n");

  free(cells);
  // printf("freeing cells finished\n");
  free_ntuple_list(points);
  // printf("freeing points finished\n");
  /*
    free( (void*) cells );
    free_ntuple_list(points);
  */
  ...
  *output_points = outArray;
  // printf("assigning output points finished\n");
  *X_out =  6;
  // printf("assigning xout finished\n");
  *N_out = n_out;
  // printf("assigning nout finished\n");
}
/*----------------------------------------------------------------------------*/
